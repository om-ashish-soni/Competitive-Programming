#include<bits/stdc++.h>
#define int long long
using namespace std;
namespace ModuloArithmetic{
    pair<int,int> extended_gcd(int a,int b){
        if(b==0) return{1,1};
        pair<int,int> pr=extended_gcd(b,a%b);
        return {pr.second,pr.first-(a/b)*pr.second};
    }
    int hcf(int a,int b){
        if(a==0 || b==0) return max(a,b);
        pair<int,int> pr=extended_gcd(a,b);
        return (a*pr.first + b*pr.second);
    }
    int lcm(int a,int b){
        return (a*b)/(__gcd(a,b));
    }
    int gcd(int a,int b){
        return __gcd(a,b);
    }
    int modSub(int a,int b,int mod){
        return ((a%mod - b%mod + mod)%mod);
    }
    int modMul(int a,int b,int mod){
        return ((a%mod)*(b%mod))%mod;
    }
    int modAdd(int a,int b,int mod){
        return ((a%mod)+(b%mod))%mod;
    }
    int modInv(int a,int mod){
        int x,y;
        pair<int,int> pr=extended_gcd(a,mod);
        x=pr.first;
        y=pr.second;
        int g=(a*x + mod*y);
        if(g!=1){
            cerr<<"modular inverse does not exist , gcd != 1"<<endl;
            exit(0);
        }
        // adding m to avoid negative value of x 
        int res=(x%mod + mod)%mod;
        return res;
    }
    int modDiv(int a,int b,int mod){
        
        a=a%mod;
        int inv=modInv(b,mod);
        int res=(inv*a)%mod;
        return res;
    }
    int power(long long x, unsigned int y, int p) 
    { 
        int res = 1; 
        x = x % p; // Update x if it is more than or 
        if (x == 0) return 0; // In case x is divisible by p;
        while (y > 0) { 
            if (y & 1) res = (res*x) % p; 
            y = y>>1; // y = y/2 
            x = (x*x) % p; 
        } 
        return res; 
    }
};
using namespace ModuloArithmetic;
class TranspositionCipher{
public:
    int cols;
    TranspositionCipher(int cols){
        this->cols=cols;
    }
    string encrypt(string msg){
        
        while(msg.size()%cols != 0){
            msg.push_back('z');
        }
        string encrypted=msg;
        int sz=msg.size();
        int rows=sz/cols;
        cout<<"rows : "<<rows<<" , cols : "<<cols<<endl;
        for(int i=0;i<rows;i++){
            for(int j=0;j<cols;j++){
                // cout<<msg[i*cols+j]<<" ";
                encrypted[j*rows+i]=msg[i*cols+j];
            }
            // cout<<endl;
        }
        return encrypted;
    }
    string decrypt(string encrypted){
        string decrypted=encrypted;
        int sz=encrypted.size();
        int rows=sz/cols;
        swap(rows,cols);
        for(int i=0;i<rows;i++){
            for(int j=0;j<cols;j++){
                // cout<<encrypted[i*cols+j]<<" ";
                decrypted[j*rows+i]=encrypted[i*cols+j];
            }
            // cout<<endl;
        }
        return decrypted;
    }
};
void solve(){
    int cols;
    cin>>cols;
    TranspositionCipher *tc=new TranspositionCipher(cols);
    string msg;
    cin>>msg;
    string encrypted=tc->encrypt(msg);
    cout<<"encrytped : "<<encrypted<<endl;
    string decrypted=tc->decrypt(encrypted);
    cout<<"decrypted : "<<decrypted<<endl;
    delete tc;
}
signed main(){
    solve();
    return 0;
}